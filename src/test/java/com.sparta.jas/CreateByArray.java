...
import com.sparta.jas.binaryTree.exceptions.DeleteRootException;
...
public class CreateByArray {
    ...
    private int[] setupArr = new int[]{5,16,3,9,18,1,14,10};
    ...
    public void testRemoveElement(){
        int[] sortedArrayAsc = arrayTree.getSortedTreeAsc();
        System.out.println(Arrays.toString(sortedArrayAsc));
        try{
            arrayTree.deleteElement(16);
        } catch (DeleteRootException e){
            e.getMessage();
        }
        int[] sortedArrayRemoved = arrayTree.getSortedTreeAsc();
        System.out.println(Arrays.toString(sortedArrayRemoved));
    }

    @Test(expected = DeleteRootException.class)
    public void testRemoveRoot() throws DeleteRootException{
        arrayTree.deleteElement(5);
    }
}
